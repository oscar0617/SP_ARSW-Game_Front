...
const checkCollision = useCallback((x, y, playerId) => {
  ...
  return players.some((otherPlayer) => {
    const player = players.find(p => p.id === playerId);
    if (otherPlayer.id === playerId || !otherPlayer.isAlive || player.isJumping) return false; 
    if (Math.floor(x) === Math.floor(otherPlayer.x) && Math.floor(y) === Math.floor(otherPlayer.y) && player.name !== 'disk') {
      ...
      crashSound();
      if(otherPlayer.id == 1 || otherPlayer.id == 2){
        red = red - 1;
        console.log("Se murio un rojo");
      }
      else{
        blue = blue - 1;
        console.log("Se murio un azul");
      }
      return true;
    }
    ...
    const updateGame = () => {
      setPlayers(prevPlayers => {
        ...
        if(blue == 0 && red == 0){
          ...
          if(blue == 0){
            setCollisionMessage('¡El Rojo gana!');
            console.log("rojo gana");
          }
          if(red == 0){
            setCollisionMessage('¡El Azul gana!');
  ...
  useEffect(() => {
    ...
    players.forEach(player => {
      ...
      // Obtener el color de la moto desde sessionStorage
      const bikeColor = sessionStorage.getItem('playersInfo')
        ? JSON.parse(sessionStorage.getItem('playersInfo')).find(p => p.id === player.id)?.color || player.color
        : player.color;
  
      // Dibujar rastro con el color inicial definido
      ctx.shadowBlur = 0; // Sin efecto de brillo para el rastro
      ctx.fillStyle = player.color + '40'; // Rastro más tenue
      player.trail.forEach(pos => {
        ctx.fillRect(pos.x * cellSize, pos.y * cellSize, cellSize, cellSize);
      });
  
      // Dibujar la moto con el color desde sessionStorage
      ctx.shadowBlur = 15; // Brillo para la moto
      ctx.shadowColor = bikeColor;
      drawPixelBike(ctx, player.x, player.y, player.direction, bikeColor, cellSize, player.isJumping, player.name);
    });
  }, [players, windowSize]);
